...
public class MyAgent implements Steppable, sim.portrayal.Orientable2D
    ...
    public MySimState theFlock;
    public boolean dead = false;
    
    public MyAgent(Double2D location) { loc = location; }
    ...
    public Double2D consistency(Bag b, Continuous2D flockers)
        ...
        for(i=0;i<b.numObjs;i++)
            {
            MyAgent other = (MyAgent)(b.objs[i]);
            if (!other.dead)
                {
                Double2D m = ((MyAgent)b.objs[i]).momentum();
                count++;
                x += m.x;
                y += m.y;
                }
    ...
    public void step(SimState state)
        {        
        final MySimState flock = (MySimState)state;
