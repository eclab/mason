...
public class DHeatBugsRMA extends SimState {
    ...
    public DDoubleGrid2DRMA valgrid;
    public DDoubleGrid2DRMA valgrid2;

    /** Creates a HeatBugs simulation with the given random number seed. */
    public DHeatBugsRMA(long seed) {
        this(seed, 1000, 1000, 1000, 2, 2, 1);
    }
    ...
    public DHeatBugsRMA(long seed, int width, int height, int count, int p_w, int p_h, int aoi) {
        super(seed);
    ...
    protected void createGrids() {
        bugs = new DHeatBugRMA[bugCount];
        valgrid = new DDoubleGrid2DRMA(gridWidth, gridHeight, p_w, p_h, pid, aoi, 0);
        valgrid2 = new DDoubleGrid2DRMA(gridWidth, gridHeight, p_w, p_h, pid, aoi, 0);
    }
    ...
    public void start() {
        ...
        for (int x = 0; x < bugCount / procCount; x++) {
            bugs[x] = new DHeatBugRMA(random.nextDouble() * (maxIdealTemp - minIdealTemp) + minIdealTemp,
    ...
    public static void main(String[] args) throws MPIException {
        MPI.Init(args);
        doLoop(DHeatBugsRMA.class, args);
        MPI.Finalize();
        System.exit(0);
    }
    ...
    private class Migrator implements Steppable {
        ...
        public void step(SimState state) {
            ...
            for (int i = 0; i < acount; i += 5) {
                DHeatBugRMA bug = new DHeatBugRMA(
