...
public /*strictfp*/ class IntGrid3D extends AbstractGrid3D
    ...
    */
     ...
     * <b>Note that the order and size of the result IntBag may not correspond to the X and Y and Z bags.</b>  If you want
     ...
     * xPos, yPos, zPos, and result IntBag, clearing them first.  
     ...
     */
    public IntBag getNeighborsAndCorrespondingPositionsMaxDistance(final int x, final int y, int z, final int dist, int mode, boolean includeOrigin, IntBag result, IntBag xPos, IntBag yPos, IntBag zPos)
        {
        if( xPos == null )
            xPos = new IntBag();
        if( yPos == null )
     ...
     * <p>You can also opt to include the origin -- that is, the (x,y) point at the center of the neighborhood -- in the neighborhood results.
     */
    public IntBag getNeighborsAndCorrespondingPositionsHamiltonianDistance(final int x, final int y, final int z, final int dist, int mode, boolean includeOrigin, IntBag result, IntBag xPos, IntBag yPos, IntBag zPos)
        {
        if( xPos == null )
            xPos = new IntBag();
        if( yPos == null )
