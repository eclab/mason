...
Java3D has a pretty limited collection of possible scene or object file types (\file{.lwo, .lws, .obj}).  However, the MASON website has a link to download {\bf NCSA Portfolio}, a free but not quite open source collection of loaders for a much wider range of files.  To use NCSA Portfolio with a BranchGroupPortrayal, you'd do something like this:
...
\begin{methods}{\class{sim.portrayal3d.LightPortrayal3D} Constructor}
...
\mthd{public LightPortrayal3D(Color color, Double3D direction, float constantAttenuation, float linearAttenuation, float quadraticAttenuation)} Creates a point light of a given color with the provided attenuation values (which range from 0.0 to 1.0 each).  You can't go wrong with 1, 0, and 0 respectively.
\mthd{public LightPortrayal3D(Light light)} Creates an arbitrary light.
\end{methods}


\paragraph{Wireframes and Axes}
