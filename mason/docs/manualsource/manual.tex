...
Removing agents can be costly: it's an \(O(n)\) search through the array to find an agent to remove.  Sequence has an optional \(O(1)\) removal procedure which has a high constant overhead but is generally far faster if, each timestep you decide to remove agents, the number of agents you remove is large (perhaps over 4).  If you're 4 or under it probably will not be worth it.  This \(O(1)\) procedure employs a \class{java.util.LinkedHashSet}, and one consequence of this is that you are not permitted to have a Sequence which has the same Steppable appearing multiple times.  Another consequence is that order is not preserved regardless of the setting of \method{setEnsuresOrder(...)}.  This option is turned on with \method{setUsesSets(true)}.
...
If you would like to subclass Sequence for some reason (very rare), here's what you need to know.  At the beginning of your \method{step(...)} call, you should first call \method{loadSteps()}. This method will rebuild (if necessary) the internal Steppable array and get it ready for you to to step.  The Steppables in question are stored like this:
...
\mthd{public boolean canEnsureOrder()} If you cannot ensure that the order of the \variable{step} array will be maintained after calling the \method{step(...)} method, override this method to return \variable{false}.
\end{methods}
