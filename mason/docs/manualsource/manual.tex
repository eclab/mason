...
public class Students extends SimState
    \{
    public Continuous2D yard = new Continuous2D(1.0,100,100);}
    public Continuous3D agitatedYard = new Continuous3D(1.0, 100, 100);
...
Here we're scheduling the anonymous agent every timestep (the 1.0) but with ordering 1, rather than the default, which is ordering 0.  This means that the agent will get stepped every timestep, but after the ordering-0 agents are stepped.  These include all the students.  That way we know the students have updated their agitation before we call the method (\method{load3DStudents()} which relocates them in the 3D field based on their agitation.
...
Java3D has a different collection of import needs than 2D Java.  Open the \file{StudentsWithUI.java} file and add the following imports:
...
\chapter{GUI and Media Utility Classes}
...
\paragraph{Random Utility Functions}  
...
PDFEncoder has a single method, which takes a \class{java.awt.Component} and a file.  It then tells the Component to repaint itself into a special Graphics object.  This Graphics object redirects all the paint directives directly into the file, to produce a PDF file. Piece of cake!
