...
3D Value Simple Portrayals are Simple Portrayals designed to represent a numerical value stored in a 3D location.  We've already seen a certain kind of 3D Value Simple Portrayal: the QuadPortrayal (Section \ref{sim.portrayal3d.grid.quad.QuadPortrayal}).  QuadPortrayals represent numerical values stored in 2D locations embedded in a mesh in 3D space, and work with ValueGrid2DPortrayal3D (which portrays 2D grids of numbers).
...
In 2D, Wrapper Simple Portrayals held subsidiary portrayals as {\it children} and added features to their portrayal of objects or otherwise manipulated them.  The same goes for the 3D case.  
...
This takes the form of a wrapper portrayal called \Class{sim.portrayal3d.SharedPortrayal3D}, which takes another SimplePortrayal as its child. When \method{getModel(...)} is called, SharedPortrayal3D will do the same {\it once} on its child, and use the result repeatedly.  It will not call \method{getModel(...)} again unless the Display3D has rebuilt the entire scene.  And you can't pick the objects.
...
Objects in fields can certainly act as their own Simple Portrayals: they need only to subclass from \class{sim.portrayal3d.SimplePortrayal3D} and implement methods as described in Section \ref{implementing3dsimple}.  

Just as is the case in 2D, if your object subclasses SimplePortrayal3D, you can still use wrapper portrayals even though there's no underlying SimplePortrayal3D registered for the object.  Just pass in \variable{null} as the child to the wrapper portrayal, and it'll use the object itself as the child.

\begin{itemize}
\item Override \method{getModel(...)} to provide an actual scene graph.  If the TransformGroup passed in is \variable{null}, a new scene graph must be built from scratch (or cloned from somewhere) and hung as a child of a new and unused TransformGroup.
\item Either make the scene graph unpickable (using \method{clearPickableFlags(...)}), or 
\item Make the scene graph pickable by using \method{setPickableFlags(...)} on each shape, and store a LocationWrapper in the user data of each shape, like this:

\java{%
LocationWrapper wrap = new LocationWrapper(object, null, parentPortrayal);\\
// ... then for each Shape3D shape...\\
shape.setUserData(wrap);
}
\end{itemize}

See the \method{getModel(...)} code in PrimitivePortrayal to get the idea.  PrimitivePortrayal only provides a model, but never changes it afterwards.  For an idea about how to change the model or its Appearance etc. over time, see the \method{getModel(...)} method in ValuePortrayal3D.
