...
\\
\\
\Large\bf Second Edition\\
\large\rm Online Version 2.0\\
\large\rm May, 2013\\
...
\small 
\noindent {\Large\bf Copyright }  2010--2013 by Sean Luke.
...
Removing agents can be costly: it's an \(O(n)\) search through the array to find the agent to remove.  Sequence has an optional \(O(1)\) removal procedure which has a high constant overhead but is generally far faster if, each timestep you decide to remove agents, the number of agents you remove is large (perhaps over 4).  If you're 4 or under it probably will not be worth it.  This \(O(1)\) procedure employs a \class{java.util.LinkedHashSet}, and one consequence of this is that you are not permitted to have a Sequence which has the same Steppable appearing multiple times.  Another consequence is that order is not preserved regardless of the setting of \method{setEnsuresOrder(...)}.  This option is turned on with \method{setUsesSets(true)}.
...
Just like Grids, Continuous space uses Maps in its methods.  The default Map employed \Class{java.util.HashMap}, but you can replace it with something else if you want.  This is a {\it rare} need.  You might do this is in the rare case that you're hunting for a more efficient Map implementation.  To replace it, you need to override the following two methods (located in SparseField):
...
Just like Grids and Continuous spaces, Network uses Maps in its methods.  The default Map employed \Class{java.util.HashMap}, but you can replace it with something else if you want.  This is a {\it rare} need.  You might do this is in the rare case that you're hunting for a more efficient Map implementation.  To replace it, you need to override the following two methods:
