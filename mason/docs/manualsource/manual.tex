...
\item \Class{sim.portrayal.inspector.BarChartChartingPropertyInspector} produces one or more bar charts of various kinds which change each iteration.  This class is capable of inspecing (and extracting its data from) any Object array consisting of Objects which fall into one or more equivalence groups (defined by the \method{equals()} method).  So as not to overwhelm JFreeChart, the number of equivalence groups may not at this time exceed 20, or else the chart will not be drawn in the inspector.
...
\mthd{public SeriesAttributes addSeries(double[] values, int bins, String name, SeriesChangeListener stopper)} Adds a series to the chart, and produces a ScatterPlotSeriesAttributes controlling that series.  The series data takes the form of a single array of doubles, forming the samples to the Histogram.  The name of the series is also provided, as is the number of bins to use in the Histogram.
...
Because of internal misfeatures in JFreeChart that are not even worth getting into, Pie Charts must provide a way for String-like objects to be unique even if they contain the same string text.  It does this with a simple utility class called \Class{sim.util.media.chart.UniqueString} which is used internally.
...
\caption{Four Bar Chart examples, showing horizontal and vertical separated charts, a stacked chart, and a stacked percentage chart.  All four examples use the same data.}
\end{figure}
...
\mthd{public PieChartSeriesAttributes(ChartGenerator generator, String name, int index, double[] values, String[] labels,} \rule{0pt}{0pt}\hspace{\fill}{\sf SeriesChangeListener stoppable)}\\
