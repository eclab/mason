...
public class AcequiaWorld extends SimState {
	...
	public void setRainfallThisYear(final int r){ rainfallThisYear = r; }
	...
	public void setBudgetThisYear(final double b){ budgetThisYear = b; budgetStdDev = b / 10; }
	...
	public void setNumRealEstateAgents( final int n ){ numRealEstateAgents = n; }
	...
	public AcequiaWorld(final long seed) {
		super(seed);
	}

	/** constructor function WITH USER-DEFINED PARAMETERS */
	public AcequiaWorld(final long seed, final int rainfallThisYear, final double acequiaAnnualDecayRate, final double acequiaCostPerUnit,
			final int [] cropPrices, final double [] parcianteStrategyProfile, final double budgetStdDev,
			final double budgetThisYear, final int numRealEstateAgents) {
	...
	public void start(){
		...
		final Ticker ticker = new Ticker(this, numberOfYearsInSimulation);
    ...
    public void finish()
    {
    	...
    	for(final Acequia a: acequiaList){
    		if( a.memberSize() == 0 )
    			defunctAcequias++;
    	}

    	// print out to some file, giving a unique name to avoid overwriting anything
		final String report = "results" + id + ".txt";

		// attempt to write out a report to this file
		try {
			// Convert our stream to a BufferedWriter
			final BufferedWriter w = new BufferedWriter(new FileWriter(report, true));

			// the header: what do all these terms mean?
			final String header = "currentTime \t rainfallThisYear \t acequiaAnnualDecayRate \t acequiaCostPerUnit" +
			...
			// the actual numbers
	    	final String output = System.currentTimeMillis() + "\t" + rainfallThisYear + "\t" + acequiaAnnualDecayRate +
	...
	void constructWaterNetwork(){
		...
		for(int i = 0; i < tiles.getWidth(); i++){
			for(int j = 0; j < tiles.getHeight(); j++){
				...
				final Tile t = (Tile) tiles.get(i, j);
				if(t == null) continue;
	...
	void setupAcequias(){
		...
		final HashMap <Integer, ArrayList <Tile>> acequias = new HashMap <Integer, ArrayList <Tile>>();
		...
		for(int i = 0; i < tiles.getWidth(); i++){
			for(int j = 0; j < tiles.getHeight(); j++){
				...
				if( acequias.containsKey( t.getAcequia() ))
					...
					final ArrayList <Tile> ts = new ArrayList <Tile> ();
					ts.add( t );
		...
		for(final Object o: acequias.keySet()){
			final Integer i = (Integer) o;
			final Acequia newAcequia = new Acequia(i, acequias.get(i));
	...
	void extractLandTractsAndAssignParciantes(){
		...
		final HashMap <Integer, ArrayList <Tile>> tractTiles = new HashMap <Integer, ArrayList <Tile>>();
		final HashMap <Integer, ArrayList <Tile>> acequiaTractTiles = new HashMap <Integer, ArrayList <Tile>>();
		final HashMap <Integer, Integer> tractToAcequiaMapping = new HashMap <Integer, Integer>();
		...
		for(final Integer i: tractTiles.keySet()){

			// assemble a list of acequia edges associated with this tract of land
			final ArrayList <Edge> myAcequiaEdges = new ArrayList <Edge> ();
			if( acequiaTractTiles.get(i) != null)
				for(final Tile t: acequiaTractTiles.get(i)){
					final Bag edges = waterflow.getEdgesOut( t );
			...
			final double strategy = random.nextDouble();
			double val = 0.;
			...
			final Parciante p = new Parciante(this, selectedStrategy, tractTiles.get(i),
			...
			if( tractToAcequiaMapping.containsKey( i ) ){
				final Acequia a = acequiaIDMap.get( tractToAcequiaMapping.get(i) );
				a.gainMember( p );
				p.setAcequia( a );
			}
		}
	 ...
	 */
	void setupIntFromFile(final String filename, final int param){
		...
		try { // to read in a file
			...
			final BufferedReader d = new BufferedReader(new InputStreamReader(AcequiasData.class.getResourceAsStream
					(filename)));
			...
			while((s = d.readLine()) != null){
				...
				for(final String p: parts){
					...
					final int value = Integer.parseInt(p);
		...
		catch (final Exception e){ e.printStackTrace();}//System.out.println(e);}
	}
	 ...
	 * @param param - the phenomenon being read in
	 */
	void setupDoubleFromFile(final String filename, final int param){
		...
		try { // to read in a file
			...
			while((s = d.readLine()) != null){
				...
				for(final String p: parts){
					...
					final double value = Double.parseDouble(p);
