...
change your mind and want to see the \class{GeomVectorField}s.  The
...
\begin{Verbatim}[frame=lines,label=Agents as Vector Field,framesep=5mm,numbers=left]
GeomVectorField boundaries = new GeomVectorField();
GeomVectorField agents = new GeomVectorField();
...
\begin{Verbatim}[frame=lines,label=Agents as Grid Field,framesep=5mm,numbers=left,commandchars=+\[\]]
GeomVectorField boundaries = new GeomVectorField();
...
\item[Solution]~\\
  ...
  can create a corresponding \idx{Java Topology Suite} (JTS)
  ...
  ``index'' along its length; i.e., you can get a JTS
...
\item[Solution]~\\
  Use the \idx{Java Topology Suite} (JTS) \class{Geometry} method
...
the data is coplanar.  Fortunately JTS has ways of breaking up lines
by their intersections.  This can be done by creating a
\class{MultiLineString} from the set of lines of interest and then
computing their \method{union()}.  The following is an example that
uses this technique to split two lines that intersect in a 'T'
formation into three line segments.
...
JTS offers a third approach to computing line intersections.  You can
...
Note that the noder returns a list of \class{LineString}s, which may
be more convenient.  Further note that the output differs from the
approach that uses \method{union()}.  The noder approach adds the line
(5 10, 5 20), which is found in reverse in the previous approach.
...
do this getting all the line coordinates, and then noting which
coordinates occur more than once; those would be the points
corresponding to line intersections.

The following code takes lines noded from the previous code example
that uses \method{union()} and prints the intersection value.

\begin{Verbatim}[frame=lines,framesep=5mm,numbers=left]
Coordinate [] vertices = brokenUpLines.getCoordinates();

Set<Coordinate> intersectionVertices = new HashSet<Coordinate>();
Set<Coordinate> visitedCoordinates = new HashSet<Coordinate>();

for (int i = 0; i < vertices.length; i++) {
    if (!visitedCoordinates.contains(vertices[i])) {
        visitedCoordinates.add(vertices[i]);
    } else {
...
for the Z coordinate, which isn't used.)


\item[Discussion]~\\
  GeoMason uses the \idx{Java Topology Suite} (JTS) to represent the
  underlying geometry and to provide geometry operations.  One such
  operation is for computing intersections.  To that end, each
  \idx{JTS} \class{Geometry} has a method, \method{intersection()},
  ...
  Note that the JTS \class{Geometry} can be directly accessed from
...
\begin{Verbatim}[frame=lines,framesep=5mm,numbers=left]
GeomVectorField agent = new GeomVectorField();
GeomGridField gridField = new GeomGridField();
...
\begin{Verbatim}[frame=lines,label=In Your \class{SimState},framesep=5mm,numbers=left,commandchars=+\[\]]
public class MyWorld extends SimState {

    // used in GUIState to set up ColorMap
    public static int NUM_AGENTS = 20;

    public GeomVectorField borders = new GeomVectorField();
...
\begin{Verbatim}[frame=lines,label=\class{MasonGeometry} subclass that
  counts agents,framesep=5mm,numbers=left,commandchars=+\[\]]
...
% TODO MASON does support alpha transparency, which would be used for
% raster overlays; but I don't think there exist a GeoMason demo that
% uses this capability.  I've temporarily commented out this recipe
% until such time that there exists a GeoMason demo showcasing raster overlays.
