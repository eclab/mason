...
However, generally we're more interested in taking a set of lines and
breaking them up by their intersections.  This can be done by creating
a \class{MultiLineString} from the set of lines of interest and then
computing their \method{union()}.  I've used the same two lines from
the previous example and break them up into four sub-lines by their
common point in the following example.
...
rawlines.add((LineString) reader.read("LINESTRING (0 10,10 10)"));
rawlines.add((LineString) reader.read("LINESTRING (5 20, 5 0)"));

MultiLineString newLines = new MultiLineString(rawlines.toArray(new LineString[0]), geometryFactory);
...
MULTILINESTRING ((0 10, 10 10), (5 20, 5 0))
Broken up by intersection: 
MULTILINESTRING ((0 10, 5 10), (5 10, 10 10), (5 20, 5 10), (5 10, 5 0))
\end{Verbatim}
