...
public class HouseholdTrailPortrayal2D extends FieldPortrayal2D
    {
    public DrawPolicy policy;
    public GUIState state;
    public HouseholdTrailPortrayal2D()
        {
        super();
        }

    /** @deprecated Use setDrawPolicy. */
    public HouseholdTrailPortrayal2D (DrawPolicy policy)
        {
        super();
        this.policy = policy;
        }
    ...
    protected void hitOrDraw(Graphics2D graphics, DrawInfo2D info, Bag putInHere)
        ...
        if (policy != null && graphics != null)
            ...
            if (graphics !=null ) // I am drawing, need to restrict
            	{
            	LocationWrapper[] wrappers = ui.display.getSelectedWrappers();
            	objects = new Bag();
            	for(int i = 0; i < wrappers.length; i++)
            		if (wrappers[i].fieldPortrayal == this)
            			objects.add(wrappers[i].getObject());
            	}
