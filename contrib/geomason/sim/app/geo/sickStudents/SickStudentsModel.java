...
public class SickStudentsModel extends SimState
{
	...
	public void setCloseAllSchoolsUponOutbreak(boolean val) { closeAllSchoolsUponOutbreak = val; }
...
//	}
	...
	public static Object getAttribute(Geometry g, String attributeName) {
		...
		int index = Collections.binarySearch(attrs, new AttributeField(attributeName), 
				GeometryUtilities.attrFieldCompartor);
		
		if (index < 0)
			return null;
		
		return ((AttributeField)(attrs.get(index))).value;
	}
	...
	private void createSchoolsFromData(GeomVectorField schoolField) {
		...
		for (int i = 0; i < geoms.numObjs; i++) {
			...
			Integer num = (Integer)getAttribute(mg, "SCHOOL_NUM");
			String name = (String)getAttribute(mg, "SCHOOL_NAM");
			String type = (String)getAttribute(mg, "SCHOOL_TYP");
...
//	}
	...
	private School findAppropriateSchool(int age, Point location) {
		...
		if (age < 11)		zones = elementarySchoolZones;
		else if (age < 15)	zones = middleSchoolZones;
		else				zones = highSchoolZones;
		...
		if (catchment.numObjs != 1) {
			...
			for (int i = 0; i < catchment.numObjs; i++)
				System.out.format("    Catchment %d: %s\n", i, getAttribute((MasonGeometry)catchment.get(i), "SCHID_3"));
			return null;
		}
		
		MasonGeometry mg = (MasonGeometry)catchment.get(0);
		Integer num = (Integer)getAttribute(mg.getGeometry(), "SCHOOL_NUM");
	...
	public void start() {
		...
		schedule.scheduleRepeating(0, 5, new Steppable() {	
			public void step(SimState state) {
				sCount = 0; iCount = 0; rCount = 0;
				for (Student s : students)
					switch (s.status) {
					case SUSCEPTIBLE: 	sCount++; break;
					case INFECTED:		iCount++; break;
					case RECOVERED:		rCount++; break;
					}
