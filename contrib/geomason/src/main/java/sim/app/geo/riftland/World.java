...
public class World extends SimState {
	...
	// <editor-fold defaultstate="collapsed" desc="Accessors">public WaterHoles
	// getWaterHoles()
...
//      initialization code for finding an almost exact number of waterholes for a given subarea
			...
			if (params.world.getWaterHoleCounterFlag()
					&& land.getSubAreaLowerRight() != null) {
			...
			weather = new Weather(params, land.getWidth(), land.getHeight(), datapath,
					random, land);
			...
			this.displacementEventDetailsCollector = new DisplacementEventDetailsCollector(
					this);
	...
	public void start() {
		...
		if (params.world.getWaterHoleCounterFlag()) {
			...
			System.out.format("WaterHoleCounts: %d\n", waterHoles
					.countWaterHoles(upperLeftX, upperLeftY, lowerRightX, lowerRightY));
			System.exit(0);
		}

		population.populate(land, waterHoles, random);
		System.out.println("Placed " + population.getCurrentNumHouseholds()
				+ " households with " + population.getCurrentPopulationFarmers()
				+ " farmers, " + population.getCurrentPopulationHerders() + " herders, "
				...
				+ (population.getCurrentPopulationFarmers()
						...
						+ population.getCurrentPopulationLaborers())
				+ " people.");
		...
		schedule.scheduleRepeating(population.getHouseHoldRandomSequence(),
				HOUSEHOLD_ORDERING, 1);
		...
		// We need to wake up the observer if we want a running data log of the
		// simulation.
//        if (params.system.isWriteStats())
		...
		if (params.system.getDisplacedDetailsOutputFilename() != null) {
			// Schedule the observer to run after everything else. (Is that right?)
			schedule.scheduleRepeating(displacementEventDetailsCollector,
					OUTPUT_DISPLACEDDETAILS, 1);
		...
		if (params.world.isScheduleMediator()) {
			...
			getLogger().log(Level.INFO, "World>start>scheduling mediator {0}",
					getMediator());
		}
