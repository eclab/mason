...
public class SchellingSpace extends SimState {
	 ...
	 */
	public SchellingSpace(final long seed) {
		super(seed);
	}

	/**
	 * Takes the geometries after they have been read in and constructs each
	 * Polygon's list of neighbors. Also extracts information about the mobile
	 * agents from the Polygons and sets up the list of Persons.
	 */
	void setup() {
		...
		for (int i = 0; i < polys.size(); i++) {
			...
			for (int k = 0; k < p1.initRed; k++) {
				...
				final Person p = new Person(Person.Affiliation.RED);
				p.region = p1;
			...
			for (int k = 0; k < p1.initBlue; k++) {
				...
				// initialize the Person
				final Person p = new Person(Person.Affiliation.BLUE);
				p.region = p1;
				p.location = randomPointInsidePolygon((Polygon) p1.geometry, geometryFactory);
				p.location.isMovable = true;
				p.location.setUserData(p);
		...
		System.out.println("Total Reds: " + totalReds);
		System.out.println("Total Blues: " + totalBlues);
	...
	public void start() {
		...
		try // to import the data from the shapefile
		{
			...
			final URL wardsFile = SchellingSpaceData.class.getResource("DCreprojected.shp");
			final URL wardsDB = SchellingSpaceData.class.getResource("DCreprojected.dbf");
