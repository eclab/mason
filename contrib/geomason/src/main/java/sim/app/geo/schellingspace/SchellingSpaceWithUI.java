...
public class SchellingSpaceWithUI extends GUIState {
	...
	protected SchellingSpaceWithUI(final SimState state) {
		super(state);
	}
	...
	public void init(final Controller controller) {
		...
		final JFrame chartFrame = happinessChart.createFrame(this);
		chartFrame.pack();
		...
		final JFrame histoFrame = numMovesHisto.createFrame(this);
		histoFrame.pack();
...
//        peoplePortrayal.setPortrayalForAll(new GeomPortrayal(Color.RED));
	...
	class MoveTracker implements Steppable {
		...
		public void step(final SimState state) {
			final SchellingSpace ps = (SchellingSpace) state;
			final int numPeople = ps.people.size();
			peoplesMoves = new double[numPeople];
			for (int i = 0; i < numPeople; i++) {
				final Person p = ps.people.get(i);
	...
	public static void main(final String[] args) {
		final SchellingSpaceWithUI worldGUI = new SchellingSpaceWithUI();
