 ...
 * 
 * The Ticker arranges the hydration, planting, harvesting, and land offering
 * subprocesses
		...
		if (simulationAge > limitYears) {
			...
			for (int i = 0; i < world.numRealEstateAgents; i++) {
				...
				RealEstateAgent re = new RealEstateAgent(
						world.random.nextGaussian() * world.budgetStdDev
								+ world.budgetThisYear);

				// pick a Parciante to offer money
				Parciante p = world.parciantes
						.get(world.random.nextInt(world.parciantes.size()));
				int offer = re.formulateOffer(p);
				boolean accepted = p.considerOffer(offer);

				if (accepted) {
					// the offer was accepted! The parciante moves and his land is
					// urbanized
			...
			if (report != null) {
				try {
					report.write(simulationAge + "\t" + world.parciantes.size() + "\t"
							+ world.formerParciantes + "\t" + world.acequiaList.size()
							+ "\t" + world.numAg + "\t" + +world.numUrban + "\t"
							+ world.numRealEstateAgents);
					report.newLine();
				} catch (IOException e) {
					e.printStackTrace();
				}
