...
import sim.app.geo.cityMigration.tinData.TinData;
/**
...
//	GeomGridField landUseGrid = new GeomGridField();
	...
	public void setCitySizeMultiplier(final double val) { citySizeMultiplier = val; }
	...
	public void setRoadNodeSizeMultiplier(final double val) { roadNodeSizeMultiplier = val; }
	...
	public void setCityIDPsSizeMultiplier(final double val) { cityIDPsSizeMultiplier = val; }
	...
	public void setMinLinksPerCity(final int val) { minLinksPerCity = val; dirty = true; }
	...
	public void setMaxLinksPerCity(final int val) { maxLinksPerCity = val; dirty = true; }
	...
	public void setThresholdLinks(final double val) { thresholdLinks = val; dirty = true; }
	...
	public void setSpatialDecayExp(final double val) { spatialDecayExp = val; dirty = true; }
	...
	public void setSimplifyRoadsDistanceExp(final double val) { simplifyRoadsDistanceExp = val; createSimpWeightedNetwork(); }
    ...
    public void setConsumptionRate(final double val) { consumptionRate = val; }
    ...
    public void setTravelPerDay(final double val) { travelPerDay = val; }

    public boolean moveOneCityPerStep = true;
    public boolean getMoveOneCityPerStep() { return moveOneCityPerStep; }
    public void setMoveOneCityPerStep(final boolean val) { moveOneCityPerStep = val; }

	public double IDPGroupSize = 0.1;
	public double getIDPGroupSize() { return IDPGroupSize; }
	public void setIDPGroupSize(final double val) { IDPGroupSize = val; }
    ...
    public void setNetworkFormationMethod(final int val) {
    	final int oldVal = networkFormationMethod;
    ...
    public void setIDPsDestinationMethod(final int val) {
...
//		propertyChangeSupport.firePropertyChange("IDPsDestinationMethod", oldVal, val);
    ...
    public void setPathTestCity1(final int val) { pathTestCity1 = val; testShortestPath(); }

    public int pathTestCity2 = 0;
    public int getPathTestCity2() { return pathTestCity2; }
    public void setPathTestCity2(final int val) { pathTestCity2 = val; testShortestPath(); }

    public int workingNetworkIndex = 2;
    public int getWorkingNetwork() { return workingNetworkIndex; }
    public void setWorkingNetwork(final int val) {
    	final int oldVal = workingNetworkIndex;
    ...
    public void setAnalyzeRoadNetwork(final boolean val) {
...
//    	workingNetwork.testAllPairsShortestPath_NextLinks();
     ...
     */
    @Deprecated
    public void setSaveAllPairsShortestPath(final boolean val) {
        ...
        final URL paths = CityMigrationData.class.getResource("roadNetworkShortestPath.ser");
     ...
     */
    @Deprecated
    public void setLoadAllPairsShortestPath(final boolean val) {
            final URL paths = CityMigrationData.class.getResource("roadNetworkShortestPath.ser");
    		//roadNetwork.loadAllPairsShortestPath(paths);
    }

    public boolean getCreateSimplifiedRoadNetwork() { return false; }
    public void setCreateSimplifiedRoadNetwork(final boolean val) {
    ...
    public void setWriteSimplifiedRoadNetwork(final boolean val) {
    ...
    public void setReadSimplifiedRoadNetwork(final boolean val) {
	...
	public void setRoadProblemDistThreshold(final double val) {
    ...
    public void setIgnoreUnconnectedPairs(final boolean val) {
    ...
    public void setTestCitiesOnly(final boolean val) {
    	testCitiesOnly = val;
		if (analyzeRoadNetwork && roadNetwork != null)
			roadNetwork.checkForRoadNetworkProblems(ignoreUnconnectedPairs, testCitiesOnly);
    }
    ...
    public void setReadDisplacementEventLog(final boolean val) { readDisplacementEventLog = val; }
	 ...
	 * @param seed
	 */
	public CityMigrationModel(final long seed) {
		super(seed);
...
//    	setCreateSimplifiedRoadNetwork(true);
	...
	private void readFiles() {
        try {
        	...
        	final GeomGridField populationGrid = new GeomGridField(); // only used for its MBR
            final InputStream populationStream = PopData.class.getResourceAsStream("riftpopulation.asc");
            ...
            final URL boundaryFile = PoliticalData.class.getResource("RiftLand_Boundary.shp");
            ...
            final URL boundaryDB = PoliticalData.class.getResource("RiftLand_Boundary.dbf");
        	...
        	final Envelope tinMBR = cityTIN.edges.getMBR();
	...
	private void initPopulationCentersFromSimplifiedRoadNetwork() {
		...
		for (final PopulationCenter pc : roadNetworkSimp.getNodes()) {
	...
	private static URL getUrl(final String nodesFilename) throws IOException {
		final InputStream nodeStream = CityMigrationModel.class.getResourceAsStream(nodesFilename);
		try {
	...
	private void init() {
    	...
    	for (final PopulationCenter pc : roadNetworkSimpWeighted.getNodes())
			pc.setRefugeeCapacity(pc.getUrbanites() * 0.1);
		...
		for (final PopulationCenter city : populationCenters.values()) {
...
//						city.getName(), city.getUrbanites(), city.urbanAreas.size(), city.getCentroid());
	...
	private void initCityNodes() {
		final boolean directed = (networkFormationMethod != 3); // everything but TIN is directed
	...
	private void buildCityNetwork() {
		...
		case 3: // TIN
			removeAllEdges(cityInteractionNetwork.edgeField);
        	final ArrayList<CityEdge> edges = cityTIN.matchEdges();
	...
	private void createSimpWeightedNetwork() {
		...
		final RoadNetwork temp = new RoadNetwork(roadNetworkSimp);
		...
		final double invExp = 1.0 / simplifyRoadsDistanceExp;
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				if (edgeMatrix[i][j] != null)
	...
	private void printNetworkStats() {
		...
		final Iterator<?> i = cityInteractionNetwork.edgeField.indexOutInHash.values().iterator();
		while (i.hasNext()) {
			final IndexOutIn ioi = (IndexOutIn)i.next();
			if (ioi.out != null)
	 ...
	 *
	 */
	private void removeAllEdges(final Network network) {
        final int n = network.allNodes.numObjs;
		final Iterator<?> i = network.indexOutInHash.values().iterator();
		for (int k = 0; k < n; k++) {
			final IndexOutIn ioi = (IndexOutIn)i.next();
	...
	private void addIDPs(final int num) {
		for (final PopulationCenter city : populationCenters.values())
			addIDPsInCity(city, num);
	}

	/** Add the given number of IDPs to the given city. */
	private void addIDPsInCity(final PopulationCenter city, int numRefugees) {
		final int householdSize = 10;
		for (; numRefugees > 0; numRefugees -= householdSize) {
			final int size = (numRefugees >= householdSize) ? householdSize : numRefugees;
			final Household h = Household.createDisplacedHousehold(params, 0, 0, size);
			city.addRefugees(h);
		}
	}

	public PopulationCenter findDestinationForIDPs(final PopulationCenter city) {
	 ...
	 */
	private PopulationCenter findDestinationForIDPs_SpareCapacity(final PopulationCenter city) {
		// Choose city
		PopulationCenter bestNeighbor = null;
		double largestSpareCapacity = Double.NEGATIVE_INFINITY;
		final Bag edges = cityInteractionNetwork.edgeField.getEdgesOut(city);
		for (int i = 0; i < edges.numObjs; i++) {
			final Edge e = (Edge)edges.get(i);
			final PopulationCenter toCity = (PopulationCenter)e.getOtherNode(city);
	 ...
	 * Find the given city's neighbor with the most spare capacity (in persons).
	 */
	private PopulationCenter findDestinationForIDPs_DistanceWeightedSpareCapacity(final PopulationCenter city) {
		final Bag edges = cityInteractionNetwork.edgeField.getEdgesOut(city);
		if (edges.numObjs == 0) {
		...
		final double[] weights = new double[edges.numObjs];
		double lowestWeight = Double.POSITIVE_INFINITY;
		for (int i = 0; i < edges.numObjs; i++) {
			final Edge e = (Edge) edges.get(i);
			final PopulationCenter toCity = (PopulationCenter) e.getOtherNode(city);
			final double dist = city.getCentroid().distance(toCity.getCentroid());
		...
		final int index = Utilities.chooseStochastically(weights, random);
		final Edge e = (Edge) edges.get(index);
	 ...
	 * city wants to send.
	 */
	private PopulationCenter findDestinationForIDPs_NearestViableCity(final PopulationCenter city) {
		final int numIDPs = (int)(city.getNumRefugees() * IDPGroupSize);

		PopulationCenter nearestViableCity = null;
		double minDistance = Double.MAX_VALUE;
		// find nearest city with spare capacity
		for (final PopulationCenter toCity : populationCenters.values())
			if ((toCity != city) && (toCity.getSpareRefugeeCapacity() >= numIDPs)) {
				final double distance = city.getCentroid().distance(toCity.getCentroid());
		...
		// send IDPs there
		final ArrayList<PopulationCenter> path = workingNetwork.getPath(city, nearestViableCity);
		if (path == null)
			return null;
	 ...
	 */
	private PopulationCenter findDestinationForIDPs_HybridTIN(final PopulationCenter city) {
		...
		final ArrayList<PopulationCenter> path = workingNetwork.getPath(city, dest);
		if (path == null)
			return null;
	 ...
	 * destination city is the first hop along that path.
	 */
	private PopulationCenter findDestinationForIDPs_HybridRoads(final PopulationCenter city) {
		final PopulationCenter dest = findDestinationForIDPs_DistanceWeightedSpareCapacity(city);
	 ...
	 */
	private PopulationCenter findDestinationForIDPs_Roads(final PopulationCenter city) {
		final Bag edges = workingNetwork.edgeField.getEdgesOut(city);
		final double[] weights = new double[edges.numObjs];
		double lowestWeight = Double.POSITIVE_INFINITY;
		for (int i = 0; i < edges.numObjs; i++) {
			final Edge e = (Edge) edges.get(i);
	 ...
	 */
	public PopulationCenter calcFirstStepOnPath(final PopulationCenter src, PopulationCenter dest) {
		if (dest == null)
			return null;
		...
		final ArrayList<PopulationCenter> path = workingNetwork.getPath(src, dest);
		if (path == null)
			return null;
	...
	private void expellIDPs(final PopulationCenter city) {
		final PopulationCenter dest = findDestinationForIDPs(city);
		if (dest == null) {
...
//		int numIDPs = (int)(city.getNumRefugees() * IDPGroupSize);
		...
		while (numIDPs > 0) {
			final RefugeeGroup rg = city.spawnRefugeeGroup(1, random);	// TODO make the number of households variable
			rg.destination = dest;
	 ...
	 */
	public PopulationCenter chooseBetterDestination(final PopulationCenter src, final PopulationCenter dest1, final PopulationCenter dest2) {
		if (dest1 == null)
			return dest2;
		if (dest2 == null)
			return dest1;
    	final double d1 = workingNetwork.getPathLength(src, dest1);
    	final double d2 = workingNetwork.getPathLength(src, dest2);
    	final double wd1 = dest1.getSpareRefugeeCapacity() / Math.pow(d1, spatialDecayExp);
    	final double wd2 = dest2.getSpareRefugeeCapacity() / Math.pow(d2, spatialDecayExp);

    	return (wd1 > wd2) ? dest1 : dest2;
	}

	public void processEvent(final DisplacementEvent event) {

//		System.out.format("Step %.2f: %s\n", schedule.getTime(), event.toString());

		// -- Find nearest city to parcelX,parcelY
		final Double2D pos = new Double2D(event.parcelX, event.parcelY);
		final Bag nearestCities = new Bag();
		...
		if (nearestCities.isEmpty())
			...
			for (int i = 0; i < nearestCities.numObjs; i++) {
				final PopulationCenter city = (PopulationCenter)nearestCities.get(i);
				final double d = pos.distance(city.getCentroid());
	...
	public void start() {
		...
		if (initialIDPsInCityID != -1) {
			final PopulationCenter city = populationCenters.get(Integer.toString(initialIDPsInCityID));
			if (city != null)
		...
		// Migration logic
		final int start = 0;
		for (final PopulationCenter _city : populationCenters.values())
	...
	private void testShortestPath() {
		try {
			...
			final PopulationCenter c1 = workingNetwork.getCity(pathTestCity1);
			if (c1 == null) {
				System.out.format("No city found with ID: %d\n", pathTestCity1);
				return;
			}
			final PopulationCenter c2 = workingNetwork.getCity(pathTestCity2);
			if (c2 == null) {
				System.out.format("No city found with ID: %d\n", pathTestCity2);
				return;
			}

			final ArrayList<PopulationCenter> path = workingNetwork.getPath(c1, c2);

			System.out.format("Path from %d to %d: ", pathTestCity1, pathTestCity2);
			if (path == null)
				System.out.println("null");
			else {
				for (final PopulationCenter c : path)
	...
	private void testNetworkList() {
		...
		final int n = workingNetwork.edgeField.getAllNodes().numObjs;
		final Edge[][] edgeMatrix = workingNetwork.edgeField.getAdjacencyMatrix();
