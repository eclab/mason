...
public class DRainfall extends DSteppable{
    ...
    public void recieveRain(DDadaab d) {
        ...
        for (int x = 0; x < d.allCamps.getWidth(); x++) {
            for (int y = 0; y < d.allCamps.getHeight(); y++) {
                ...
                DFieldUnit field = (DFieldUnit) d.allCamps.getLocal(new Int2D(x, y));
    ...
    public void drain(DDadaab d) {
        ...
        for (int x = 0; x < d.allCamps.getWidth(); x++) {
            for (int y = 0; y < d.allCamps.getHeight(); y++) {
                ...
                // ceter cell
                //DFieldUnit field = (DFieldUnit) d.allCamps.field[x][y];
                DFieldUnit field = (DFieldUnit) d.allCamps.getLocal(new Int2D(x,y));
...
//    }
    ...
    public void fieldDrainageSimple(DFieldUnit field, DDadaab d){
         Bag n = new Bag();
                   n.clear();
                // get moore neighborhood
                ...
                d.allCamps.getMooreLocations(field.getX(), field.getY(), 1, 0, false, xPos, yPos);
                
                for (int i=0; i<xPos.numObjs; i++) {
                	DFieldUnit my_obj = d.allCamps.getLocal(new Int2D(xPos.get(i), yPos.get(i)));
                	n.add(my_obj);
                }
    ...
    public void waterAbsorbtion(DDadaab d) {
    ...
    public void drawRiver(DDadaab d) {
        ...
        for (int x = 0; x < d.allCamps.getWidth(); x++) {
            for (int y = 0; y < d.allCamps.getHeight(); y++) {
                ...
                DFieldUnit field = (DFieldUnit) d.allCamps.getLocal(new Int2D(x,y));
                d.rainfallGrid.set(new Int2D(field.getX(), field.getY()), field.getWater());
