...
import sim.app.geo.ddadaab.DCampBuilder.Node;
...
public class DRefugee extends DSteppable implements Valuable, java.io.Serializable {
	    ...
	    private DFieldUnit position; // current position
	    private DFieldUnit home;// home 
	    private DFieldUnit goal; // location of the goal
...
//	    private double protectiveImmunity; // after recovery agent will not likely infected immediately
	    ...
	    //
	    DTimeManager tm;// time contorler-identify the hour, day, week
	    ArrayList<DFieldUnit> path = null; // the agent's current path to its current goal
	    MersenneTwisterFast randomN ;

	    public DRefugee(int age, int sex, DFamily hh, DFieldUnit home, DFieldUnit position, MersenneTwisterFast random, DContinuous2D allRefugees) {
	        this.setAge(age);
	        this.setSex(sex);
	        this.setFamily(hh);
	        this.setHome(home);
...
//	        stayingTime = 0;
	        ...
	        Double2D loc = new Double2D(hh.getCampLocation().getX() + jitterX, hh.getCampLocation().getY() + jitterY);	        
	        allRefugees.moveAgent(loc, this);
	        
	    }

	    private void setPosition(DFieldUnit position) {
	        this.position = position;

	    }

	    public DFieldUnit getPosition() {
	        return position;
	    }

	    // goal position - where to go
	    public void setGoal(DFieldUnit position) {
	        this.goal = position;

	    }

	    public DFieldUnit getGoal() {
	        return goal;
	    }

	    // home location   
	    public void setHome(DFieldUnit home) {
	        this.home = home;

	    }

	    public DFieldUnit getHome() {
	        return home;
	    }
	    ...
	    // faimly memeber
	    public void setFamily(DFamily hh) {
	        this.hh = hh;
	    }
...
//	              
	    ...
	    public void move(int steps) {
	        ...
	        if (this.getGoal() == null) {
	        	...
	        	//System.out.println("double check position and goal are both global");
	        	//System.exit(-1);
...
//	                           make sure we have a path to the goal!
	            if (path == null || path.size() == 0) {
	                path = DAStar.astarPath(d,
	                        (Node) d.closestNodes.get(new Int2D(this.getPosition().getX(), this.getPosition().getY())),
	                        (Node) d.closestNodes.get(new Int2D(this.getGoal().getX(), this.getGoal().yLoc)));
	                if (path != null) {
	            ...
	            DActivity current = new DActivity();
	            DFieldUnit loc = current.getNextTile(d, subgoal, this.getPosition());

	            
	            /*
	            DFieldUnit oldLoc = this.getPosition();
	            ...
	            */
                ...
                Double2D toPos = new Double2D(loc.getX(), loc.getY());
	            d.allRefugees.moveAgent(toPos, this);
	 ...
	 /*
	    ...
	    public int actSelect() {
	        ...
	        if (!isDayTime) {
	            ...
	            return (wHealth < 0.3) ? DActivity.STAY_HOME : DActivity.HEALTH_CENTER;
	        } else {
	    ...
	    public void featchWater(DFieldUnit f) {
...
//	        }
	     ...
	     */
	    public void doActivity(DFieldUnit f, int activ) {

	        switch (activ)
	        {
	            default:
	            case DActivity.STAY_HOME:
	                break;
	                
	            case DActivity.BOREHOLE_OR_RIVER:
...
//	                }
	            ...
	            case DActivity.HEALTH_CENTER:
	                recieveTreatment(f, d);
	                break;
	                
	            case DActivity.SOCIAL_RELATIVES:
	                
	                 if(randomN.nextDouble()< d.params.global.getProbabilityGuestContaminationRate()){
	                     additionalWater(f);
	                 }
	                 break;
	            case DActivity.VISIT_SOCIAL:
	                if(randomN.nextDouble() < d.params.global.getProbabilityGuestContaminationRate()){
	                     additionalWater(f);
	                 }
	                break;
	                
	            case DActivity.VISIT_LATRINE:
	                useLatrine(f);
...
//	                      
	    ...
	    public void useLatrine(DFieldUnit f) {
...
//	            this.transimissionCholera(d);// may be get contaminated -due to contaminated laterine
	...
	//  
	    public void additionalWater(DFieldUnit f) {
	        ...
	        // if you visit other camp or friends, drking some water from there
	            if(f.getRefugeeHH().isEmpty() == true){
	                ...
	                if ( ((DFamily) f.getRefugeeHH().objs[r]).getWaterAtHome() > 2) {
	                
	                cv = ((DFamily) f.getRefugeeHH().objs[r]).getWaterrBacteriaLevel()/1000;
	                double water = ((DFamily) f.getRefugeeHH().objs[r]).getWaterAtHome();
//	                System.out.println("cv: - "+ cv);
	                ((DFamily) f.getRefugeeHH().objs[r]).setWaterAtHome(water - 2);
	    ...
	    public void recieveTreatment(DFieldUnit f, DDadaab d) {
