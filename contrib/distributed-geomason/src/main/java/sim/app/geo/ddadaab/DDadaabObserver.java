...
public class DDadaabObserver extends DSteppable{
  ...
  DDadaabObserver(DDadaab dadaab)
    {
//    	setup(world);
    ...
    private void startLogFile()
    {
        ...
        {
            Logger.getLogger(DDadaab.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void step(SimState state)
    {
        ...
        String numTotAgent  = Integer.toString(new Bag(d.allRefugees.getAllAgentsInStorage()).numObjs);
...
//        // writeGrid =true;
        ...
        {
            Logger.getLogger(DDadaabObserver.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//  
  public void writeCholeraSpread(){
     ...
     DDoubleGrid2D grid = new DDoubleGrid2D(d);
      // first put all values zero
            ...
            for (int i = 0; i < d.allCamps.getWidth(); i++)
            {
                ...
                //DFieldUnit faci = (DFieldUnit) d.allCamps.get(i, j);
                DFieldUnit faci = (DFieldUnit) d.allCamps.getLocal(new Int2D(i, j));

                if(faci.getCampID() > 0){
                    grid.setLocal(new Int2D(i,j), 0);
                }
              
            }
        }
     // then write the current refugee health status
      for(Object o:d.allRefugees.getAllAgentsInStorage()){
          DRefugee r = (DRefugee)o;
          //double tot = grid.field[r.getPosition().getX()][r.getPosition().getY()] ;
          double tot = grid.getLocal(new Int2D(r.getPosition().getX(),r.getPosition().getY())) ;

          if(r.getHealthStatus()==3){
             //grid.field[r.getPosition().getX()][r.getPosition().getY()] = tot + 1;
             grid.setLocal(new Int2D(r.getPosition().getX(),r.getPosition().getY()), tot + 1);
          }
          else{
              //grid.field[r.getPosition().getX()][r.getPosition().getY()] = tot;
              grid.setLocal(new Int2D(r.getPosition().getX(),r.getPosition().getY()), tot);
