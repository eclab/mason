...
import sim.app.geo.dhaiti.DHaitiFood.Node;
 ...
 */
class DAgent extends DSteppable {
	 ...
	 */
	public DAgent(Location home, Location position, int destruction) {
		this.home = home;
	...
	public DAgent(Location home, Location position, int destruction, double enToStay, 
	 ...
	 */
	void checkGoal(DHaitiFood state) {
...
//				goal.distanceTo(position) * ENERGY_TO_WALK_PAVED < energyLevel ){
	...
	public void step(SimState state) {
		...
		DHaitiFood hf = (DHaitiFood) state;
		...
		// moves, it expends more or less energy
			...
			Location nextTile = null;
			try {
				nextTile = getNextTile(hf, subgoal);
	 ...
	 */
	double tileCost(Location loc, DHaitiFood hf) throws RemoteException{
		Bag people = hf.population.getObjectsAtLocation(loc.x, loc.y);
		if( people != null && people.size() > hf.riotDensity) return ENERGY_TO_RIOT;
		int roadType = hf.roadsGrid.get(new Int2D(loc.x, loc.y)).getInt();
	 ...
	 * @throws RemoteException 
	 */
	Location getNextTile(DHaitiFood world, Location subgoal) throws RemoteException {
		...
		boolean xmoveToRoad = ((Integer)world.roadsGrid.get(new Int2D(xmove.x, xmove.y)).getInt()) > -1;
		boolean ymoveToRoad = ((Integer)world.roadsGrid.get(new Int2D(ymove.x, ymove.y)).getInt()) > -1;
