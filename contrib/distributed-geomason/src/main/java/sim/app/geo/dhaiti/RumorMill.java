 ...
 *
 */
public class RumorMill extends DSteppable {
	...
	IntGrid2D distributeInfo(SparseGrid2D population, IntGrid2D oldInfo, 
		...
		for( int i = 0; i < gridWidth; i++){
			for( int j = 0; j < gridHeight; j++){
				...
				if( agents != null)
					for(Object o: agents){
						((DAgent)o).centerInfo = neighborInfoTile;
					}
			}
		}
		return neighborInfo;
	...
	public void step(SimState state) {
		...
		SparseGrid2D population = ((DHaitiFood)state).population;
		...
		IntGrid2D infoFromNeighbors = assembleInfoFromNeighbors( info, ((DHaitiFood)state).locations );
