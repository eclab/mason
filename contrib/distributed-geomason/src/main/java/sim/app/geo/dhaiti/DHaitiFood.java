...
public class DHaitiFood extends DSimState {
	private static final long serialVersionUID = 1L;
	public static final int width = 300;
	public static final int height = 300;
	public static final int aoi = 2; // TODO what value???
	public static final int discretization = 6;
	
//	// for peopleList?
//	public final DContinuous2D<DAgent> peopleLocations;
//	
//	// for population?
//	// TODO no DSparseGrid2D, so using DDenseGrid2D for now
//	public DDenseGrid2D<DAgent> populationD;
    ...
    public DIntGrid2D roadsGrid; //TODO not distributed
    ...
    ArrayList<DAgent> peopleList = new ArrayList<DAgent>();
...
//		peopleLocations = new DContinuous2D<>(discretization, this);
//        populationD = new DObjectGrid2D<>(this);
    }
...
//        populationD = new DObjectGrid2D<>(this);
    ...
    public void start(){
        ...
        try {
            ...
            DIntGrid2D tempPop = setupFromFile(popFile, 0);
    ...
    void populate(DIntGrid2D pop) throws RemoteException {
        ...
        for (int i = 0; i < pop.getWidth(); i++) {
            ...
            for (int j = 0; j < pop.getHeight(); j++) {
                ...
                ArrayList<DAgent> ps = new ArrayList<DAgent>();

                // get the population
                int tile = pop.get(new Int2D(i, j)).getInt();
                ...
                if (tilePop > 0) {
                    ...
                    for (int x = 0; x < Math.min(intPop, 12); x++) {
                        DAgent a;
                        if(interval < 0)
                            a = new DAgent(here, here.copy(), destructionLevel);
                        else
                            a = new DAgent(here, here.copy(), destructionLevel, enToStay, 
        ...
        if(interval < 0){
            enToStay = DAgent.ENERGY_TO_STAY;
            enWalkPaved = DAgent.ENERGY_TO_WALK_PAVED;
            enWalkUnpav = DAgent.ENERGY_TO_WALK_UNPAVED;
            enRiot = DAgent.ENERGY_TO_RIOT;
        }
     ...
     */
    DIntGrid2D setupFromFile(URL filename, int defaultValue) {

        DIntGrid2D field = null;
    ...
    public static void main(String[] args) {
    	doLoopDistributed(DHaitiFood.class, args);
        System.exit(0);
    }
