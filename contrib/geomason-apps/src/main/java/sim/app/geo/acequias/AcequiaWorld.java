...
import sim.app.geo.acequias.data.AcequiasData;
...
public class AcequiaWorld extends SimState {
	...
	int numberOfYearsInSimulation = 20; // how long should the simulation run?
	int id = -1; // used to identify output files that belong together
	
	// --- OBJECTS ---
	
	// --- Land Objects ---
	SparseGrid2D tiles = null;
	...
	public void start(){
		...
		// read in the data
		System.out.println("read in elevations");
		setupDoubleFromFile("ned.txt", 1);

		System.out.println("read in land use");
		setupIntFromFile("landuse.txt", 3);

		System.out.println("read in hydrology");
		setupIntFromFile("hydrology.txt", 1);
		
		System.out.println("read in acequia tracts");
		setupIntFromFile("acequiatracts.txt", 2);		

		System.out.println("read in acequias");
		setupIntFromFile("acequias.txt", 4);
		
		System.out.println("read in roads");
		setupIntFromFile("roads.txt", 5);

		System.out.println("read in counties");
		setupIntFromFile("counties.txt", 6);

		System.out.println("read in city center");
		setupIntFromFile("citycenter.txt", 7);

		// finished reading in the data
		
		System.out.println("Finished reading in the data.\nBeginning data setup...");
		
		// set up the data

		System.out.println("building the acequia network...");
		setupAcequias();
	...
	void constructWaterNetwork(){
		...
		for(int q = 0; q < b.size(); q++)
			...
			Int2D loc = (Int2D)(tiles.getObjectLocation(t));
			int i = loc.x;
			int j = loc.y;
	...
	public SparseGrid2D getTiles(){ return tiles; }
...
//			FileInputStream fstream = new FileInputStream(filename);
			InputStream fstream = AcequiasData.class.getResourceAsStream(filename);
			...
			if ( tiles == null )
				tiles = new SparseGrid2D( width, height);
			...
			while((s = d.readLine()) != null){
				...
				Tile t = null;
				Bag b = tiles.getObjectsAtLocation(j,i);
				if (b == null || b.size() == 0)
					{
					t = new Tile(j, i);
					tiles.setObjectLocation(t, j, i);
					}
				else
					{
					t = (Tile)(b.get(0));
					...
					if( param == 1 ) // hydrology
						t.setHydrologicalFeature(value);
