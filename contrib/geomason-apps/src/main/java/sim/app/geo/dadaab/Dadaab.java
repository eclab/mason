...
public class Dadaab extends SimState {
	...
	public Dadaab(final long seed, final String[] args) {
		super(seed);
	...
	public void start() {
		...
		try {
			CampBuilder.create(DadaabData.class.getResource("d_camp_a.txt"),
					DadaabData.class.getResource("d_faci_a.txt"),
					DadaabData.class.getResource("d_costp_a.txt"),
					this, random);
		...
		final Steppable chartUpdater = new Steppable() {

			// all graphs and charts wll be updated in each steps
			public void step(final SimState state) {

				final Bag ref = allRefugees.getAllObjects(); // getting all refugees
				//
				...
				for (int i = 0; i < allFamilies.numObjs; i++) {
					final Family f = (Family) allFamilies.objs[i];
					// killrefugee(f);
				...
				for (int i = 0; i < ref.numObjs; i++) {
					final Refugee r = (Refugee) ref.objs[i];
				...
				totalTotalPopSeries.add((state.schedule.time()), allRefugees.getAllObjects().numObjs);
	...
	public void killrefugee(final Refugee f) {
	...
	public int countDeath() {
		...
		final int current = allRefugees.getAllObjects().numObjs;
		PrevPop = curPop;
