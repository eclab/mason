...
final public class Population {
	...
	public void setDisplacedPeopleByEthnicity(
			HashMap<Integer, Integer> displacedPeopleByEthnicity) {
	...
	public void addDisplacementEvent(DisplacementEvent event) {
		World.getLogger().log(Level.FINE,
				String.format("Household displaced: %s", event.toString()));
		...
		if (!displacedPeopleByEthnicity.containsKey(event.culture))
			...
			displacedPeopleByEthnicity.put(event.culture,
					event.groupSize + displacedPeopleByEthnicity.get(event.culture));
	}
	...
	public int getCurrentPopulationFarmers() {
		...
		// return householdbag.size(); // this won't work because we don't remove
		// households
		int total = 0;
		for (int i = 0; i < householdsbag.size(); i++) {
			Household thisagent = (Household) householdsbag.objs[i];
	...
	private int getNewHouseholdSize(MersenneTwisterFast random) {
		// Get a random household size with specified mean (m), sampled from [2/3m,
		// 4/3m].
		double twoThirdsMean = (2.0 / 3.0) * params.households.getMeanHouseholdSize();
		int householdPop;
		householdPop = (int) Math
				.round(twoThirdsMean + random.nextDouble() * twoThirdsMean);
	...
	public double calculateCurrentPopulationChangeRate() {
		int totalPop = getCurrentPopulationFarmers() + getCurrentPopulationHerders()
				+ getCurrentPopulationLaborers() + getCurrentPopulationDisplaced();
		// System.out.println( " world>calculateCurrentPopulationChangeRate> total pop =
		// " + totalPop);
		...
		{
			changeRate = ((double) (totalPop - getLastTotalPop()) * 365.0 * 100.0)
					/ (double) totalPop;
		} else {
			changeRate = 0;
		}
		// System.out.println( " world>calculateCurrentPopulationChangeRate> last pop =
		// " + getLastTotalPop());
		// System.out.println( " world>calculateCurrentPopulationChangeRate> pop change
		// = " + (totalPop - getLastTotalPop()));

		setLastTotalPop(totalPop);

		setCurrentPopulationChangeRate(changeRate);
		// System.out.println( " world>calculateCurrentPopulationChangeRate> change rate
		// = " + changeRate);
...
//        assert coveringObjects.size() == 1 : x + " " + y + " " + coveringObjects.size();
	...
	public Population(Parameters params, String datapath, Land land) {
		...
		try {
			...
			BufferedInputStream populationStream;
			if ("".equals(datapath))
				populationStream = new BufferedInputStream(RiftlandData.class
						.getResourceAsStream(params.world.getPopulationFile()));
			else
				populationStream = new BufferedInputStream(
						new FileInputStream(populationFileName));
			...
			ArcInfoASCGridImporter.read(populationStream,
					GeomGridField.GridDataType.INTEGER, populationGrid);

		} catch (IOException ex) {
			World.getLogger().log(Level.SEVERE, null, ex);
		}
		Logger.getLogger(World.class.getName())
				.info("Population: Finished Loading data...");
		assert (repOK());
	}
	...
	private void loadEthnicRegions(String dataPath) {
		...
		try {
			...
			URL murdockDB;
			if ("".equals(dataPath)) {
				murdockURL = RiftlandData.class
						.getResource(params.world.getEthnicRegionsFile());
				murdockDB = RiftlandData.class.getResource(
						params.world.getEthnicRegionsFile().replace("shp", "dbf"));
			} else {
				File murdockFile = new File(
						dataPath + params.world.getEthnicRegionsFile());
				murdockURL = murdockFile.toURI().toURL();
				File murdockDBF = new File(dataPath
						+ params.world.getEthnicRegionsFile().replace("shp", "dbf"));
			...
			ShapeFileImporter.read(murdockURL, murdockDB.toURI().toURL(), ethnicRegions,
					masked);

			System.out.println("Loaded " + ethnicRegions.getGeometries().numObjs
					+ " ethnic regions");
		} catch (Exception ex) {
			World.getLogger().log(Level.SEVERE, null, ex);
		}
	 ...
	 *                       ethnicRegions
	 */
	private void createEthnicIDMap(GeomVectorField ethnicRegions,
			Map<Integer, String> ethnicIDToName) {
	...
	public void populate(final Land land, final WaterHoles waterHoles,
		...
		if (params.world.isOnlyOneHousehold()) {
			...
			if (!isValidHouseholdSite(land, xLoc, yLoc))
				throw new IllegalStateException(
						"Population.populate: Attempted to place only one household on an invalid location.");
			...
			Household newHousehold = new Household(params, waterHoles, this, location,
					farmSize, cultureID, countryID, size, random);
	 ...
	 * becomes feasable.
	 */
	private void placeHouseholdsUsingPopulationData(final Land land,
			final WaterHoles waterHoles, final MersenneTwisterFast random) {
		...
		class RowPopulator implements Runnable {
			...
			RowPopulator(int row, long seed) {
				this.row = row;
				localRandom = new MersenneTwisterFast(seed); // MersenneTwisterFast is not
																// threadsafe, so we chain
																// PRNG's.
			}

			@Override
			public void run() {
				for (int x = land.getSubAreaUpperLeft().x; x < land
						.getSubAreaLowerRight().x; x++) {
					if (isValidHouseholdSite(land, x, row))
						populateParcel(x, row, land, localRandom, waterHoles);
				}
				Logger.getLogger(World.class.getName())
						.info(String.format("Row %d populated", row));
			}
		}

		Logger.getLogger(World.class.getName()).info(
				"Entering Population.placeHouseholdsInRegionUsingData(World world)");
		householdsGrid.clear(); // Clear out any households from any previous runs.
		if (params.world.getInitTruePopulationPortion() <= 0)
			return;

		ExecutorService executor = Executors
				.newFixedThreadPool(params.system.getNumthreads());
		for (int y = land.getSubAreaUpperLeft().y; y < land
				.getSubAreaLowerRight().y; y++) {
	...
	private void populateParcel(int x, int y, Land land, MersenneTwisterFast random,
			WaterHoles waterHoles) {
		final int desiredParcelPop = (int) Math
				.round(((IntGrid2D) populationGrid.getGrid()).get(x, y)
						* params.world.getInitTruePopulationPortion());
		...
		while (parcelPopSoFar < desiredParcelPop) {
			...
			// If this is the last household, cull its size so we don't overfill the
			// parcel.
			...
			// We will come back and reset all the farm sizes once we know how many
			// households are placed on this parcel.
			final int initialFarmSize = 0;
			Household newHousehold = new Household(params, waterHoles, this, location,
					initialFarmSize, cultureID, countryID, householdPop, random);
		...
		if (!newHouseholds.isEmpty()) {
			if (params.households.canFarm()) {
				...
				for (Household h: newHouseholds) {
					...
					h.setFarmAreaInHectares(
							Math.min(maxSizeInHectares, h.getFarming().getPopulation()));
	...
	final public boolean repOK() {
		assert (!(params.system.isRunExpensiveAsserts() && !Misc
				.containsOnlyType(householdsGrid.getAllObjects(), Household.class)));
		assert (!(params.system.isRunExpensiveAsserts()
				&& !Misc.containsOnlyType(herdingGrid.getAllObjects(), Herding.class)));
		assert (!(params.system.isRunExpensiveAsserts()
				&& !Misc.containsOnlyType(farmingGrid.getAllObjects(), Farming.class)));

		return params != null && populationGrid != null && householdsGrid != null
				&& herdingGrid != null && farmingGrid != null && ethnicRegions != null
				&& houseHoldRandomSequence != null && ethnicIDToName != null
				&& displacementEvents != null && displacedPopulation >= 0
				&& !(params.system.isRunExpensiveAsserts()
						&& !Misc.containsOnlyType(householdsGrid.getAllObjects(),
								Household.class))
				&& !(params.system.isRunExpensiveAsserts() && !Misc
						.containsOnlyType(herdingGrid.getAllObjects(), Herding.class))
				&& !(params.system.isRunExpensiveAsserts() && !Misc
						.containsOnlyType(farmingGrid.getAllObjects(), Farming.class));
	}
}
