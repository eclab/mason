...
public class ConflictDiamonds extends SimState {
	...
	// PECS parameters
	public enum Motive {
		Basic_Needs, Maintain_Current_Lifestyle, Better_Life
	};

	public enum Goal {
		Stay_Home, Find_Employment_As_Miner, Remain_Employed, Rebel
	};

	public enum Action {
		Move_Closer_to_Diamond_Mines_Mine, Move_Close_to_Diamond_Mines_Rebel, Do_Nothing
	};
	...
	public void setTotalbyRegion(int[] val) {
		totalbyRegion = val;
	}

	public int[] getTotalbyRegion() {
		return totalbyRegion;
	}

	private int[] totalAction;

	public void setTotalAction(int[] val) {
		totalAction = val;
	}
	...
	public void start() {
		...
		// create the grids
		try {
			ConflictDiamondsBuilder.create("z_landscape.txt", "z_cities.txt",
					"z_diamonds.txt", "z_diamondcities.txt", "z_population.txt", this);
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(-1);
		}
		Steppable chartUpdater = new Steppable() {
			public void step(SimState state) {
				...
				for (int i = 0; i < allResidents.size(); i++) {
					...
					if (r.getResidingParcel().getRegion().getRegionID() == 1) {
						sumTotalbyRegion[0] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 2) {
						sumTotalbyRegion[1] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 3) {
						sumTotalbyRegion[2] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 4) {
						sumTotalbyRegion[3] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 5) {
						sumTotalbyRegion[4] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 6) {
						sumTotalbyRegion[5] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 7) {
						sumTotalbyRegion[6] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 8) {
						sumTotalbyRegion[7] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 9) {
						sumTotalbyRegion[8] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 10) {
						sumTotalbyRegion[9] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 11) {
						sumTotalbyRegion[10] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 12) {
						sumTotalbyRegion[11] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 13) {
						sumTotalbyRegion[12] += 1;
					}
					if (r.getResidingParcel().getRegion().getRegionID() == 14) {
						sumTotalbyRegion[13] += 1;
					}
				...
				sumActions[2] = sumActions[2]
						+ diamondMinerEmployer.getEmployees().size();
				...
				for (int i = 0; i < sumActions.length; i++) {
					dataset.setValue(
							sumActions[i] * 100
									/ (allResidents.size() + allRebels.size()),
							actTitle, activities[i]);
				}
			}
		};
