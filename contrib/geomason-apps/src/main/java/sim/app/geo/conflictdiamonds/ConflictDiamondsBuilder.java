...
public class ConflictDiamondsBuilder {
	 ...
	 *
	 * @param regionfile           - identifies the modeling world and boundaries.
	 *                             Source of data is the Global Administrative Areas
	 *                             (GADM, 2009)
	 * @param remotenessfile       - each cell is assigned a remoteness value (based
	 *                             on distance from cities and highways). Source of
	 *                             data is OpenStreetMap (OpenStreetMap, 2010)
	 * @param diamondfile          - the location of diamond mines (represents real
	 *                             world locations). Source of data is the Peace
	 *                             Research Institute Oslo (PRIO) Center for the
	 *                             Study of Civil War (Gilmore et al., 2005)
	 * @param diamondproximatefile - the location of diamond mines if assumed to be
	 *                             in Freetown
	 * @param populationfile       - the population by cell. Source of data is Oak
	 *                             Ridge National Laboratory (2007).
	 * @param conflictDiamonds
	 * 
	 *
	 */
	public static void create(String regionfile, String remotenessfile,
			String diamondfile, String diamondproximatefile, String populationfile,
			ConflictDiamonds conflictDiamonds) throws Exception {
		try {
			// Open the file

			// Convert our input stream to a BufferedReader
			BufferedReader regionarea = new BufferedReader(new InputStreamReader(
					ConflictDiamondsData.class.getResourceAsStream(regionfile)));
			BufferedReader remotesurface = new BufferedReader(new InputStreamReader(
					ConflictDiamondsData.class.getResourceAsStream(remotenessfile)));
			BufferedReader diamondsurface = new BufferedReader(new InputStreamReader(
					ConflictDiamondsData.class.getResourceAsStream(diamondfile)));
			BufferedReader diamondproximatesurface = new BufferedReader(
					new InputStreamReader(ConflictDiamondsData.class
							.getResourceAsStream(diamondproximatefile)));
			BufferedReader populationdist = new BufferedReader(new InputStreamReader(
					ConflictDiamondsData.class.getResourceAsStream(populationfile)));
			...
			URL boundariesShapeUL = ConflictDiamondsData.class
					.getResource("z_boundaries.shp");
//            		getUrl("/conflictdiamonds/conflictdiamondsData/z_boundaries.shp");//file.toURL();
			// System.exit(0);
			URL boundariesShapeDB = ConflictDiamondsData.class
					.getResource("z_boundaries.dbf");
//            		getUrl("/conflictdiamonds/conflictdiamondsData/z_boundaries.dbf");//file.toURL();
			ShapeFileImporter.read(boundariesShapeUL, boundariesShapeDB,
					conflictDiamonds.allBoundaries, regionNames);

			Bag diamondLocation = new Bag();
			diamondLocation.add("Country");

			URL diamondsShapeUL = ConflictDiamondsData.class
					.getResource("z_Diamond_SL.shp");
//            		getUrl("/conflictdiamonds/conflictdiamondsData/z_Diamond_SL.shp");//fileDiamonds.toURL();
			URL diamondsShapeDB = ConflictDiamondsData.class
					.getResource("z_Diamond_SL.dbf");
//            		getUrl("/conflictdiamonds/conflictdiamondsData/z_Diamond_SL.dbf");//fileDiamonds.toURL();

			ShapeFileImporter.read(diamondsShapeUL, diamondsShapeDB,
					conflictDiamonds.allDiamonds, diamondLocation);
			...
			while ((line = populationdist.readLine()) != null) {
				...
				for (String p: parts) {
					...
					for (i = 0; i < value; i++) {
						// if select to place rebels in regions of the country that most
						// closely match where the oppositon group was formed
						if (conflictDiamonds.params.global
								.isInitialRebelPositionActualEvents()
								&& conflictDiamonds.random
										.nextDouble() < conflictDiamonds.params.global
												.getOppositionActual()) {
							// if running experiment where diamond mines are in Freetown
							if (conflictDiamonds.params.global.isProximateExperiment()) {
								if (par.getRegion().getRegionID() == 12
										|| par.getRegion().getRegionID() == 13
										|| par.getRegion().getRegionID() == 6) {
									// create and initialize initial rebels
									Rebel newRebel = new Rebel(conflictDiamonds, par,
											region);
									conflictDiamonds.allRebels.add(newRebel);
									newRebel.setResidingParcel(par);
									par.addPopulation(newRebel);
									conflictDiamonds.allPopulation.setObjectLocation(
											newRebel, par.getX(), par.getY());
									...
									newRebel.getResidingRegion()
											.addInitialRebel(newRebel);
							...
							// if running experiment where diamond mines are distant (in
							// actual locations)
							else {
								if (par.getRegion().getRegionID() == 3
										|| par.getRegion().getRegionID() == 1) {
									// create and initialize initial rebels
									Rebel newRebel = new Rebel(conflictDiamonds, par,
											region);
									conflictDiamonds.allRebels.add(newRebel);
						...
						// if select to place initial rebels (opposition group) at random
						// locations
						else if (conflictDiamonds.params.global
								.isInitialRebelPositionActualEvents() == false
								&& conflictDiamonds.random
										.nextDouble() < conflictDiamonds.params.global
												.getOpposition()) {
							// create and initialize initial rebels
							...
							newRebel.setResidingParcel(par);
							par.addPopulation(newRebel);
							conflictDiamonds.allPopulation.setObjectLocation(newRebel,
									par.getX(), par.getY());
							conflictDiamonds.schedule.scheduleOnce(newRebel);
							newRebel.setOpposition(true);
						...
						// otherwise, agent is not an initial rebel, so create a resident
						// agent
						else {
							// create and initialize residents
							Resident newResident = new Resident(conflictDiamonds, par,
									region);
							conflictDiamonds.allResidents.add(newResident);
							newResident.setResidingParcel(par);
							par.addPopulation(newResident);
							conflictDiamonds.allPopulation.setObjectLocation(newResident,
									par.getX(), par.getY());
							...
							if (newResident.getIncomeLevel() == 0) {
								region.addFoodPoor(newResident);
							} else if (newResident.getIncomeLevel() == 1) {
								region.addTotalPoor(newResident);
							} else {
								region.addNotPoor(newResident);
							}

							// add residents to region objects based on employment and
							// income attributes for tracking and reporting purposes
							if (newResident.getOtherEmployer() != null) {
								region.addFormalEmployee(newResident);
							}
							if (newResident.getDiamondMiner() != null) {
								region.addInformalEmployee(newResident);
							}
							if (newResident.isMinor()) {
								region.addMinors(newResident);
							}
							if (newResident.isActiveLaborForce()) {
								region.addActiveLaborMarket(newResident);
							}
							if (newResident.isEligible()) {
								region.addEligibleToMine(newResident);
							}
			...
			Logger.getLogger(ConflictDiamondsBuilder.class.getName()).log(Level.SEVERE,
					null, ex);
		}
	}
	...
	private static URL getUrl(String nodesFilename) throws IOException {
		try {
			InputStream nodeStream = ConflictDiamondsBuilder.class
					.getResourceAsStream(nodesFilename);
	 ...
	 * 
	 * Source for agent's age is from Sierra Leone's 2004 population and housing
	 * census, age and sex structure report (Thomas et al., 2006)
	 * Labor attributes are from Sierra Leone's 2004 population and housing census,
	 * employment and labor report (Braima et al., 2006)
	 * Income levels are from Statistics Sierra Leone’s Annual Statistical Digest
	 * (2006)
	 *
	 * @param r                - the region
	 * @param conflictDiamonds
	...
	public static void setupRegions(Region r, ConflictDiamonds conflictDiamonds) {
		...
		double percentEmployedFormal; // proportion of total population that is employed
										// aged 15-64
		double percentLaborForce; // proportion of population that is active aged 15-64
		...
		if (r.getRegionID() == 1)
			percentFoodPoor = .45;
		else if (r.getRegionID() == 2)
			percentFoodPoor = .38;
		else if (r.getRegionID() == 3)
			percentFoodPoor = .22;
		else if (r.getRegionID() == 4)
			percentFoodPoor = .63;
		else if (r.getRegionID() == 5)
			percentFoodPoor = .09;
		else if (r.getRegionID() == 6)
			percentFoodPoor = .29;
		else if (r.getRegionID() == 7)
			percentFoodPoor = .2;
		else if (r.getRegionID() == 8)
			percentFoodPoor = .32;
		else if (r.getRegionID() == 9)
			percentFoodPoor = .25;
		else if (r.getRegionID() == 10)
			percentFoodPoor = .35;
		else if (r.getRegionID() == 11)
			percentFoodPoor = .16;
		else if (r.getRegionID() == 12)
			percentFoodPoor = .14;
		else if (r.getRegionID() == 13)
			percentFoodPoor = .15;
		else if (r.getRegionID() == 14)
			percentFoodPoor = .02;
		else
			percentFoodPoor = .0;
		...
		if (r.getRegionID() == 1)
			percentEmployedFormal = .27;
		else if (r.getRegionID() == 2)
			percentEmployedFormal = .36;
		else if (r.getRegionID() == 3)
			percentEmployedFormal = .39;
		else if (r.getRegionID() == 4)
			percentEmployedFormal = .35;
		else if (r.getRegionID() == 5)
			percentEmployedFormal = .30;
		else if (r.getRegionID() == 6)
			percentEmployedFormal = .34;
		else if (r.getRegionID() == 7)
			percentEmployedFormal = .35;
		else if (r.getRegionID() == 8)
			percentEmployedFormal = .29;
		else if (r.getRegionID() == 9)
			percentEmployedFormal = .38;
		else if (r.getRegionID() == 10)
			percentEmployedFormal = .38;
		else if (r.getRegionID() == 11)
			percentEmployedFormal = .38;
		else if (r.getRegionID() == 12)
			percentEmployedFormal = .31;
		else if (r.getRegionID() == 13)
			percentEmployedFormal = .46;
		else if (r.getRegionID() == 14)
			percentEmployedFormal = .39;
		else
			percentEmployedFormal = .0;
		...
		if (r.getRegionID() == 1)
			percentLaborForce = .33;
		else if (r.getRegionID() == 2)
			percentLaborForce = .38;
		else if (r.getRegionID() == 3)
			percentLaborForce = .41;
		else if (r.getRegionID() == 4)
			percentLaborForce = .37;
		else if (r.getRegionID() == 5)
			percentLaborForce = .36;
		else if (r.getRegionID() == 6)
			percentLaborForce = .38;
		else if (r.getRegionID() == 7)
			percentLaborForce = .38;
		else if (r.getRegionID() == 8)
			percentLaborForce = .35;
		else if (r.getRegionID() == 9)
			percentLaborForce = .36;
		else if (r.getRegionID() == 10)
			percentLaborForce = .36;
		else if (r.getRegionID() == 11)
			percentLaborForce = .39;
		else if (r.getRegionID() == 12)
			percentLaborForce = .35;
		else if (r.getRegionID() == 13)
			percentLaborForce = .41;
		...
		r.setLaborStats(percentEmployedFormal, percentLaborForce, percent0to4,
				percent5to6, percent7to14, percent15to17, percent18to64, percent65Over,
				percentMining);
	}
