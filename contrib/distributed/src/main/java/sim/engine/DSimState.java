...
public class DSimState extends SimState
    ...
    public DSimState(long seed, int width, int height, int aoi, boolean isToroidal)
        {
        super(seed, new MersenneTwisterFast(seed), new DSchedule());
                    ...
                    if (!haloGrid2D.getLocalBounds().contains(storageBound))
                        ...
                        for (Object a : agents)
                            ...
                            if (a instanceof Stopping && !migratedAgents.contains(a) && oldPartition.contains(loc)
                                ...
                                if (stopping.getStoppable() instanceof DTentativeStep)
                                    {
                                    DTentativeStep step = (DTentativeStep) stoppable;
                                    stoppable.stop();
                                ...
                                // stop agent in schedule, then migrate it
                                else if (stopping.getStoppable() instanceof DIterativeRepeat)
                                    {
                                    final DIterativeRepeat step = (DIterativeRepeat) stopping.getStoppable();
                                    stoppable.stop();
                        ...
                        if (aList != null)
                            ...
                            for (int i = ((ArrayList<Serializable>) aList).size() - 1; i >= 0; i--)
                                ...
                                if (a != null && a instanceof Stopping && !migratedAgents.contains(a) && oldPartition.contains(p) && !partition.getLocalBounds().contains(p))
                                    ...
                                    // stop agent in schedule, then migrate it
                                    if (stoppable instanceof DTentativeStep)
                                        {
                                        DTentativeStep step = (DTentativeStep) stoppable;
                                        stoppable.stop();
                                    ...
                                    // stop and migrate
                                    else if (stoppable instanceof DIterativeRepeat)
                                        {
                                        final DIterativeRepeat step = (DIterativeRepeat) stopping.getStoppable();
                                        stoppable.stop();
