...
public class DPSO extends DSimState {
	...
	protected void startRoot() {
        ...
        final DParticle p = new DParticle(x, y, vx, vy, this, f, i);
        particles[i] = p;
	...
	public void start() {
		...
		for (Object p : particles) {
			...
			if (partition.getLocalBounds().contains(a.position)) {
				  ...
				  this.space.moveAgent(new Double2D(a.position), a); //get this in storage
				  
		          schedule.scheduleRepeating(Schedule.EPOCH,1,new Steppable()
	                {
	                public void step(SimState state) { a.stepUpdateFitness(); }
	                });
	                        
	            schedule.scheduleRepeating(Schedule.EPOCH,2,new Steppable()
	                {
	                public void step(SimState state) { a.stepUpdateVelocity(); }
	                });
	                        
	            schedule.scheduleRepeating(Schedule.EPOCH,3,new Steppable()
	                {
	                public void step(SimState state) { a.stepUpdatePosition(); }
	                });		}
		}
        ...
        schedule.scheduleRepeating(Schedule.EPOCH, 5, new Steppable()
        {
        public void step(SimState state)
            {
