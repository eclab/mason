...
\mthd{public List getEntitiesAsList()}
	Returns in a java.util.List all the current Entities the Provider can provide.  You can
        modify the array (it's yours), but do not modify the Entities stored inside, as they
        are the actual Entities stored in the Provider.  If this Provider does not provide
        Entities, then null is returned.  
...
RandomQueue overrides a number of methods, but two are worth mentioning here:

\begin{methods}{\class{sim.des.RandomQueue} Constructor}
\mthd{public RandomQueue(SimState state, Entity typical)}
Builds a RandomQueue with the given state and typical provided and received entity.  Note that RandomQueue can only have Entities (or their subclasses) as its typical resources.
\end{methods}

\begin{methods}{\class{sim.des.RandomQueue}}
\mthd{public boolean accept(Provider provider, Resource amount, double atLeast, double atMost)}
...
A Queue, but restricted only to Entities.  When a PriorityQueue needs to offer an Entity, it selects the one with the lowest Comparable value from among its existing pile of Entities.  PriorityQueue cannot receive nor offer non-Entity Resources.

This implies that each Entity must be associated with a Comparable.  By default the Comparable is assumed to be the Entity's ``info'' object, and if this object is null or is not Comparable, an exception is thrown.  However you can override this to define any Comparable object you wish.

Because it only uses Entities, not CountableResources, and because it must provide the Entities in an efficient manner, PriorityQueue does not use either the {\tt CountableResource resource} nor {\tt LinkedList \(<\)Entity\(>\) entities} instance variables from Provider.  {\tt resource} is set to {\tt null}, and {\tt entities} is a real LinkedList but is kept empty and ignored: do not use it in subclasses.

PriorityQueue overrides a number of methods, but three are worth mentioning here:

\begin{methods}{\class{sim.des.PriorityQueue} Constructor}
\mthd{public PriorityQueue(SimState state, Entity typical)}
Builds a PriorityQueue with the given state and typical provided and received entity.  Note that PriorityQueue can only have Entities (or their subclasses) as its typical resources.
\end{methods}
